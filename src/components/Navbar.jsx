...
// Global coming soon trigger
export function triggerComingSoon(pageName) {
  window.dispatchEvent(new CustomEvent('show-coming-soon', { detail: { page: pageName } }))
}

export default function Navbar({ onStaffPortal = () => {} }) {
  ...
  return (
    <>
        ...
        <div className="max-w-[1280px] mx-auto px-10">
          <div className="flex items-center justify-between h-[72px]">
            ...
            <ul className="hidden lg:flex items-center gap-7 list-none">
              {NAV_LINKS.map(link => (
                <li key={link.label} className="relative group h-[72px] flex items-center">
                  <a href={link.href || '#'}
                    onClick={(e) => {
                  ...
                  {/* Desktop Dropdown */}
                  {link.dropdown && DRAWER_SUBS[link.label] && (
                    <div className="absolute top-[72px] left-0 w-64 bg-white shadow-xl rounded-b-xl border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-2 group-hover:translate-y-0 z-50">
                      <div className="py-2">
                        {DRAWER_SUBS[link.label].map(sub => (
                          <button
                            key={sub.label}
                            onClick={() => handleSubLink(sub)}
                            className="w-full text-left px-5 py-2.5 text-[14px] font-medium text-gray-600 hover:text-brand-red hover:bg-gray-50 flex items-center justify-between transition-colors">
                            {sub.label}
                            {!sub.href && (
                              <span className="text-[9px] font-heading font-bold text-brand-red bg-red-50 px-1.5 py-0.5 rounded tracking-wide">SOON</span>
                            )}
                          </button>
                        ))}
                      </div>
            ...
            {/* ── Actions (Staff Portal + Book) ───────────────────── */}
            <div className="flex items-center gap-4">
              
              {/* Staff Portal - Desktop */}
              <button
                onClick={onStaffPortal}
                className="hidden lg:flex items-center gap-1.5 text-[13px] font-medium text-gray-500 hover:text-brand-red transition-colors font-heading tracking-wide">
      ...
      <aside
        ...
        <div className="px-6 py-5 border-t border-white/[0.08] flex flex-col gap-3">
          <a href="#booking"
            onClick={() => setDrawerOpen(false)}
          ...
          <button
            onClick={() => { setDrawerOpen(false); onStaffPortal(); }}
            className="flex items-center justify-center gap-2 w-full font-heading font-semibold text-white/50
              hover:text-white text-[13px] py-2 transition-colors">
